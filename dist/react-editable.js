	...
	(function (factory) {
	            ...
	            if (sel.type == 'Control') // e.g. an image selected
	                {
	                    ...
	                    var range = sel.createRange();
	                    for (var i = 0; i < range.length; ++i) nodes.push(range(i));
	                    return nodes;
	                }
	        }
	        return [];
	    ...
	    var expandSelectionCaret = function expandSelectionCaret(containerNode, preceding, following) {
	        if (window.getSelection) {
	            var sel = window.getSelection();
	            if (sel.modify) {
	                for (var i = 0; i < preceding; ++i) sel.modify('extend', 'backward', 'character');
	                for (var i = 0; i < following; ++i) sel.modify('extend', 'forward', 'character');
	            } else {
	    ...
	    */
	    ...
	    var wysiwyg = function wysiwyg(option) {
	        ...
	        // Command structure
	        var callUpdates = function callUpdates(selection_destroyed) {
